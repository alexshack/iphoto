<div>
    ...
    <div class="page-header d-xl-flex d-block">
        <div class="page-leftheader">
            <h4 class="page-title">#{{ $expense->id }} от {{ $expense->date }}<a href="{{url('money/expenses')}}" class="font-weight-normal text-muted ml-2">Расходы ДС</a></h4>
        </div>
    </div>
    <!--End Page header-->
                            ...
                            <div class="col-md-9">
                                <select wire:model="expense.expense_type_id" class="form-control select2-show-search custom-select" data-placeholder="Выберите вид расхода">
                                    ...
                                    @foreach($expenseTypes as $type)
                                        <option @if($expense->expense_type_id === $type->id) selected @endif value="{{ $type->id }}">
                                        {{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-9">
                                <select wire:model="expense.city_id" class="form-control select2-show-search custom-select" data-placeholder="Выберите город">
                                    ...
                                    @foreach($cities as $city)
                                        <option value="{{ $city->id }}" {{ $expense->city_id === $city->id ? 'selected' : '' }}>
                                            {{ $city->name }}
                                        </option>
                                    @endforeach
                                </select>
                                            ...
                                            <div class="col-md-9">
                                                <select wire:model="expense.place_id" class="form-control select2-show-search custom-select" data-placeholder="Выберите точку">
                                                    ...
                                                    @foreach($places as $place)
                                                        <option value="{{ $place->id }}" {{ $expense->place_id === $place->id ? 'selected' : '' }}>
                                                         {{ $place->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-md-9">
                                                <select wire:model="expense.manager_id" class="form-control select2-show-search custom-select" data-placeholder="Выберите менеджера">
                                                    ...
                                                    @foreach($managers as $manager)
                                                        <option value="{{ $manager->id }}" {{ $expense->manager_id === $manager->id ? 'selected' : '' }}>
                                                            {{ $manager->getFullName() }}
                                                        </option>
                                                    @endforeach
                                                </select>
